...
export class UsersController {
  ...
  async createCustomer(
    ...
    if (result.isExisting) {
      ...
      const isAlreadyInStore = await this.checkIfCustomerInStore(
  ...
  private async checkIfCustomerInStore(
    customerId: string,
    storeId: string,
  ): Promise<boolean> {
    try {
      // Use the existing getStoreCustomers method to check if customer is in store
      const storeCustomers = await this.transactionsService.getStoreCustomers(
        storeId,
        { role: "store", storeId },
      );
  ...
  async findAll(
    ...
    const response = await this.usersService.findAll(listRequest, {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Get available filter options for users (Admin only)",
  })
  @ApiResponse({
    status: 200,
  ...
  async getCurrentUser(@CurrentUser() user: any): Promise<UserResponseDto> {
    ...
    const fullUser = await this.usersService.findByPhoneNumber(
      user.phoneNumber,
    );
    if (!fullUser) {
