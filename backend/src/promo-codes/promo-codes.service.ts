...
export class PromoCodesService {
  ...
  private transformPromoCodeToResponse(
    ...
    return {
      ...
      user:
        populatedPromoCode.populated("userId") && populatedPromoCode.userId
          ? {
  ...
  async findAll(
    ...
    return {
      data: promoCodes.map((promoCode) =>
        this.transformPromoCodeToResponse(promoCode),
      ),
      total,
      page,
      limit,
  ...
  async updateStatus(
    id: string,
    changeStatusDto: ChangePromoCodeStatusDto,
    user: any,
  ): Promise<PromoCodeResponseDto> {
    const promoCode = await this.promoCodeModel.findById(id).exec();
    if (!promoCode) {
  ...
  async validateCode(
    ...
    if (!promoCode) {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_NOT_FOUND,
    ...
    if (promoCode.status === "deleted") {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_DELETED,
    ...
    if (promoCode.status === "used") {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_ALREADY_USED,
    ...
    if (!promoCode.userId) {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_NOT_REGISTERED,
    ...
    if (promotion.storeId.toString() !== storeId) {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_INVALID_STORE,
    ...
    if (user.role === "store" && user.storeId !== storeId) {
      return {
        isValid: false,
        message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_FORBIDDEN_STORE,
    ...
    return {
      ...
      message: PERSIAN_ERROR_MESSAGES.PROMO_CODE_IS_VALID,
    };
  }

  async bulkCreate(
    bulkCreateDto: BulkCreatePromoCodesDto,
    user: any,
  ): Promise<PromoCodeResponseDto[]> {
    ...
    return createdCodes.map((code) =>
      this.transformPromoCodeToResponse(code as PromoCodeDocument),
    );
  }
  ...
  private generatePromoCodeWithPrefix(
    prefix?: string,
  ...
  async registerCodeToUser(
    ...
    // Create transaction record (CRITICAL: Every promo code registration must create a transaction)
    const transaction = new this.transactionModel({
      customerId: user._id,
      storeId: promotion.storeId,
      promoCodeId: promoCode._id,
  ...
  async getUserPromoCodes(
    ...
    if (requestingUser?.role === "store" && storeId) {
      ...
      const userStore = await this.storeModel
        .findOne({ userId: requestingUser._id })
        .exec();
  ...
  async getStats(
    ...
    const [total, unused, used, registered, deleted] = await Promise.all([
      ...
      this.promoCodeModel
        ...
        .exec(),
      this.promoCodeModel
        .countDocuments({ ...query, status: "deleted" })
        .exec(),
    ]);
  ...
  async sendOtpForPromoRegistration(
    registerDto: RegisterWithPromoCodeDto,
  ...
  async verifyPromoRegistration(
    verifyDto: VerifyPromoRegistrationDto,
    ...
    let user = await this.userModel
      .findOne({ phoneNumber: verifyDto.phoneNumber })
      .exec();
