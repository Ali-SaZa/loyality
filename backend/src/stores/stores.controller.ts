...
export class StoresController {
  ...
  @ApiResponse({
    status: 409,
    description: "Store with this phone number already exists",
  })
  ...
  async createStoreWithUser(
    @Body() createStoreWithUserDto: CreateStoreWithUserDto,
  ...
  @ApiOperation({
    summary:
      "Update own store information (Store Owner only) - Restricted fields",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Update store information (Store Owner/Admin only)",
  })
  @ApiParam({ name: "id", description: "Store ID" })
  @ApiResponse({
    status: 200,
  ...
  async updateSmsBalance(
    ...
    return this.storesService.updateSmsBalance(
      id,
      updateSmsBalanceDto.amount,
      user,
    );
  }
  ...
  @ApiResponse({
    status: 403,
    description: "Forbidden - user is not a customer of the store",
  })
  ...
  async sendSmsToCustomerFromMyStore(
    ...
    if (user.role === "store") {
      return this.storesService.sendSmsToCustomer(
        user.storeId,
        sendSmsToCustomerDto.userId,
        sendSmsToCustomerDto.text,
        user,
      );
    }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Get SMS history for current store (Store Owner only)",
  })
  @ApiQuery({
    name: "page",
    required: false,
