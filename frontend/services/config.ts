...
export const API_ROUTES = {
  ...
  SIMULATIONS: {
    ...
    GET_BY_ID: (simulationId: string) =>
      `job-simulations/visitor/${simulationId}`,
    GET_ALL_REVIEW_QUESTIONS: "job-simulation/review-questions/learner-all",
    GET_SIMULATION_QUIZ: (simulationId: string) =>
      `job-simulation/${simulationId}/quiz`,
  },
  JOB_CATEGORIES: {
    GET_ALL_ROOT: (parentId?: string) =>
      `job-categories/all/visitor${parentId ? `?parentId=${parentId}` : ""}`,
  },
  ORGANIZATIONS: {
    GET_ALL: "organizations/visitor",
    GET_BY_ID: (organizationId: string) =>
      `organization/${organizationId}/visitor`,
  },
  TASKS: {
  ...
  SIMULATION_USER: {
    INSERT_TASK: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}/task`,
    REGISTER: "job-simulation-user/register",
    GET_USER_SIMULATIONS: "job-simulation-users/learner",
    GET_USER_SIMULATION_BY_ID: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}`,
    SAVE_USER_COMMENT: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}/comment`,
    CHAT_WITH_EVALUATOR: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}/chat`,
    SAVE_USER_REVIEW_FOR_SIMULATION: "job-simulation-user/review",
    GET_SIMULATION_COMMENTS: (jobSimulationId: string) =>
      `job-simulation-user/${jobSimulationId}/comments/visitor`,
    COMPLETE_QUIZ: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}/complete-quiz`,
    GET_EVALUATION_COST: (jobSimulationUserId: string) =>
      `job-simulation-user/${jobSimulationUserId}/evaluation-cost`,
...
export const HANDLE_ERROR = (error: any) => {
  const message =
    error.response?.data?.message || "خطایی در انجام درخواست پیش آمد.";
