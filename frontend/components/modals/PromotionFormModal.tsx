...
const PromotionFormModal = ({
  ...
  const methods = useForm<CreatePromotionData | UpdatePromotionData>({
    resolver: zodResolver(
      isEditMode ? UpdatePromotionValidation : CreatePromotionValidation,
    ),
    defaultValues: {
      storeId: "",
      title: "",
    ...
    try {
      ...
      if (isEditMode && promotionId) {
        ...
        await updatePromotion(
          promotionId,
          transformedData as UpdatePromotionRequest,
        );
      } else {
        // Create new promotion
        const createdPromotion = await createPromotion(
          transformedData as CreatePromotionRequest,
        );

        // Get store name for the created promotion
        const selectedStore = stores.find(
          (store) => store.id === transformedData.storeId,
        );
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-info-50 border border-info-200 rounded-lg">
              <p className="text-info text-sm">
                <strong>مثال:</strong> اگر مبلغ خرید ۱۰۰,۰۰۰ تومان و امتیاز ۱
                باشد، مشتری با خرید ۱۰۰,۰۰۰ تومان، ۱ امتیاز دریافت می‌کند.
              </p>
            </div>
          </div>
        </FormProvider>
