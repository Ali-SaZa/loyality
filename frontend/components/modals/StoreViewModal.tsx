...
const StoreViewModal = ({ isOpen, onOpenChange, onEdit, onDelete, onSuccess, storeId }: StoreViewModalProps) => {
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-text-dark">{store.name}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="border-1">
                ...
                <CardBody className="space-y-4">
                  <div>
                    <label className="text-sm text-text-light">نام فروشگاه</label>
                    <p className="font-medium">{store.name}</p>
                  </div>
                  <div>
                    <label className="text-sm text-text-light">شماره تلفن</label>
                    <p className="font-medium">{formatPhoneNumber(store.phoneNumber)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="text-sm text-text-light">تعداد تبلیغات</label>
                    <p className="font-medium">{store.promotions?.length || 0} تبلیغ فعال</p>
                  </div>
                  {store.description && (
                    <div>
              ...
              <Card className="border-1">
                <CardHeader className="pb-3">
                  <h3 className="text-lg font-semibold text-text-dark">آدرس و اطلاعات تماس</h3>
                </CardHeader>
                <CardBody className="space-y-4">
                  <div>
                    <label className="text-sm text-text-light">شهر و استان</label>
                    <p className="font-medium">{getAddressText(store.address)}</p>
                  </div>
                  <div>
                  ...
                  {store.workingHours && (
                    <div>
                      <label className="text-sm text-text-light">ساعات کاری</label>
                  ...
                  {store.socialLinks && (
                    <div>
                      <label className="text-sm text-text-light">شبکه‌های اجتماعی</label>
                      <div className="space-y-1">
                        {store.socialLinks.website && (
                          <p className="text-sm">وب‌سایت: {store.socialLinks.website}</p>
                        )}
                        {store.socialLinks.instagram && (
                          <p className="text-sm">اینستاگرام: {store.socialLinks.instagram}</p>
                        )}
                        {store.socialLinks.telegram && (
                          <p className="text-sm">تلگرام: {store.socialLinks.telegram}</p>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <CardBody className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {store.planExpiryDate && (
                    <div>
                      <label className="text-sm text-text-light">تاریخ انقضای طرح</label>
                      <p className="font-medium">{formatDate(store.planExpiryDate)}</p>
                    </div>
                  )}
                </div>
              </CardBody>
