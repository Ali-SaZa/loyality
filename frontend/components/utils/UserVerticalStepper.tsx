...
const UserVerticalStepper = ({ currentStep, stepsDetail }: StepperProps) => {
  ...
  useEffect(() => {
    if (activeStepRef.current && containerRef.current) {
      ...
      if (activeStepRect.top >= containerRect.top && activeStepRect.bottom <= containerRect.bottom) {
        container.scrollTo({
          top: scrollPosition,
          behavior: 'smooth',
        })
      ...
      {stepsDetail.map((item, index) => (
        ...
        >
          <div className="flex flex-col items-center">
            ...
            {index !== data.length - 1 && (
              <div className={`w-[3px] h-[86px] ${currentStep > index ? 'bg-primary' : 'bg-background-50'} rounded-xl my-1`} />
            )}
          </div>
          <div className="flex flex-col gap-3 mt-1">
            <p className={`font-bold leading-6 ${isActiveStep(index) ? 'text-primary-100' : 'text-text'}`}>ماموریت {index + 1}</p>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <div className={`${currentStep >= index ? 'bg-primary' : 'bg-background-50'} size-2 min-w-2 min-h-2 rounded-full`} />
                <p className={`${isActiveStep(index) ? 'text-primary' : 'text-text-light-25'} text-sm leading-4`}>{item.title}</p>
              </div>
              <div className="flex items-center gap-2">
                <div className={`${currentStep >= index ? 'bg-primary' : 'bg-background-50'} size-2 min-w-2 min-h-2 rounded-full`} />
                <p className={`${isActiveStep(index) ? 'text-primary' : 'text-text-light-25'} text-sm leading-4`}>
              ...
              <div className="flex items-center gap-2">
                <div className={`${currentStep >= index ? 'bg-primary' : 'bg-background-50'} size-2 min-w-2 min-h-2 rounded-full`} />
                <p className={`${isActiveStep(index) ? 'text-primary' : 'text-text-light-25'} text-sm leading-4`}>
                  {data.taskDifficultyLevels.find((dl) => dl.code === item.difficultyLevel)?.name}
                </p>
              </div>
            </div>
          </div>
