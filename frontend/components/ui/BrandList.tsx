...
const BrandList = () => {
  ...
  return (
    <div className="overflow-hidden">
      <div ref={scrollRef} className="flex space-x-4 p-4 whitespace-nowrap">
        <motion.div animate={controls} className="flex space-x-16">
          {brands.map((brand, index) => (
            <div key={index} className="shrink-0">
              <img alt={`Brand ${index}`} className="h-32 w-auto" src={brand} />
            </div>
          ))}
          {brands.map((brand, index) => (
