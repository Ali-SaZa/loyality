...
export const getStatusConfig = (status: string) => {
  return (
    STATUS_CONFIG[status as UserStatus] || STATUS_CONFIG[UserStatus.ACTIVE]
  );
};

export const getStorePlanConfig = (planType: string) => {
  return (
    STORE_PLAN_CONFIG[planType as StorePlanType] ||
    STORE_PLAN_CONFIG[StorePlanType.FREE]
  );
};

export const getStoreStatusConfig = (status: string) => {
  return (
    STORE_STATUS_CONFIG[status as StoreStatus] ||
    STORE_STATUS_CONFIG[StoreStatus.ACTIVE]
  );
};

export const getPromotionTypeConfig = (type: string) => {
  return (
    PROMOTION_TYPE_CONFIG[type as PromotionType] ||
    PROMOTION_TYPE_CONFIG[PromotionType.POINTS_BASED]
  );
};

export const getPromotionStatusConfig = (status: string) => {
  return (
    PROMOTION_STATUS_CONFIG[status as PromotionStatus] ||
    PROMOTION_STATUS_CONFIG[PromotionStatus.ACTIVE]
  );
};

export const getPromoCodeStatusConfig = (status: string) => {
  return (
    PROMO_CODE_STATUS_CONFIG[status as PromoCodeStatus] ||
    PROMO_CODE_STATUS_CONFIG[PromoCodeStatus.UNUSED]
  );
};

export const getScratchCardStatusConfig = (status: string) => {
  return (
    SCRATCH_CARD_STATUS_CONFIG[status as ScratchCardStatus] ||
    SCRATCH_CARD_STATUS_CONFIG[ScratchCardStatus.ACTIVE]
  );
};

export const getScratchCardTypeConfig = (type: string) => {
  return (
    SCRATCH_CARD_TYPE_CONFIG[type as ScratchCardType] ||
    SCRATCH_CARD_TYPE_CONFIG[ScratchCardType.FIXED]
  );
};
...
export const getReportStatusConfig = (status: string) => {
  return (
    ...
    REPORT_STATUS_CONFIG[ReportStatus.COMPLETED]
  );
};

export const getReportTypeConfig = (type: string) => {
  return (
    REPORT_TYPE_CONFIG[type as ReportType] ||
    REPORT_TYPE_CONFIG[ReportType.DAILY]
  );
};

export const getTransactionTypeConfig = (type: string) => {
  return (
    TRANSACTION_TYPE_CONFIG[type as TransactionType] ||
    TRANSACTION_TYPE_CONFIG[TransactionType.PURCHASE]
  );
};

export const getTransactionStatusConfig = (status: string) => {
  return (
    TRANSACTION_STATUS_CONFIG[status as TransactionStatus] ||
    TRANSACTION_STATUS_CONFIG[TransactionStatus.COMPLETED]
  );
};
