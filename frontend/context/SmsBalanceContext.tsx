...
const SmsBalanceContext = createContext<SmsBalanceContextType | undefined>(
  undefined,
);
...
export const SmsBalanceProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <SmsBalanceContext.Provider
      value={{ smsBalance, loading, error, refetch: fetchSmsBalance }}
    >
      {children}
    </SmsBalanceContext.Provider>
  );
...
export const useSmsBalanceContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useSmsBalanceContext must be used within a SmsBalanceProvider",
    );
  }
  return context;
};
