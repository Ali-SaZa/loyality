...
export const convertPersianToEnglish = (str: string): string => {
  ...
  return persianNumbers.reduce(
    (acc, persianNum, index) => acc.replace(persianNum, englishNumbers[index]),
    str,
  );
};
    ...
    (acc, [key, value]) => {
      if (
        ...
        (!(typeof value === "object" && !Array.isArray(value)) ||
          Object.keys(value).length) // بررسی آبجکت‌های خالی
      ) {
        acc[key] =
          typeof value === "object" && !Array.isArray(value)
...
export const convertToISOFormat = (dateObject: any) => {
  ...
  const date = dayjs(
    new Date(gy, gm - 1, gd, hour || 0, minute || 0, second || 0),
  );
...
export const formatDateToCustomTimezone = (
  date: Date,
  timezoneOffset = 210,
): string => {
